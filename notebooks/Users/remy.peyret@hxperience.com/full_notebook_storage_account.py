...
spark.conf.set(
  "fs.azure.account.key.hxdatastorageblob.blob.core.windows.net",
  "8u6LgHe2I57WzHZj5AB8+/N4ZDkJSDEi+uBrjs412n4s7OqqRneoHmhBMMwnAhVwfuUGjSzdj1qLpjvJvPpvMA==")

postgres_path = "wasbs://postgresqlcopy@hxdatastorageblob.blob.core.windows.net"
print(dbutils.fs.ls(postgres_path))

bimodels_path = "wasbs://bi-models@hxdatastorageblob.blob.core.windows.net"
print(dbutils.fs.ls(bimodels_path))
...
tenants = spark.read.format('csv').options(header='true', inferschema='true').load(postgres_path + "/public.tenancy_tenant.txt")
for r in tenants.select('name').collect():
  dirname = bimodels_path + '/' + str(r.name) + '/' + 'a.txt'
...
all_tables = dbutils.fs.ls(postgres_path)
for t in all_tables:
...
device = spark.read.format('csv').options(header='true', inferschema='true').load(postgres_path + "/public.core_device.txt")
entity = spark.read.format('csv').options(header='true', inferschema='true').load(postgres_path + "/public.core_entity.txt")
entitytype = spark.read.format('csv').options(header='true', inferschema='true').load(postgres_path + "/public.core_entitytype.txt")
equipment = spark.read.format('csv').options(header='true', inferschema='true').load(postgres_path + "/public.core_equipment.txt")
equipmenttemplate = spark.read.format('csv').options(header='true', inferschema='true').load(postgres_path + "/public.core_equipmenttemplate.txt")
fluidtype = spark.read.format('csv').options(header='true', inferschema='true').load(postgres_path + "/public.core_fluidtype.txt")
functionalgroup = spark.read.format('csv').options(header='true', inferschema='true').load(postgres_path + "/public.core_functionalgroup.txt")
location = spark.read.format('csv').options(header='true', inferschema='true').load(postgres_path + "/public.core_location.txt")
tag = spark.read.format('csv').options(header='true', inferschema='true').load(postgres_path + "/public.core_tag.txt")
transceiver = spark.read.format('csv').options(header='true', inferschema='true').load(postgres_path+ "/public.core_transceiver.txt")
transceivertype = spark.read.format('csv').options(header='true', inferschema='true').load(postgres_path + "/public.core_transceivertype.txt")
unit = spark.read.format('csv').options(header='true', inferschema='true').load(postgres_path + "/public.core_unit.txt")
usage = spark.read.format('csv').options(header='true', inferschema='true').load(postgres_path + "/public.core_usage.txt")
usagetype = spark.read.format('csv').options(header='true', inferschema='true').load(postgres_path + "/public.core_usagetype.txt")
kpi = spark.read.format('csv').options(header='true', inferschema='true').load(postgres_path + "/public.kpi_kpi.txt")
...
entity.write.partitionBy('depth').save(bimodels_path + '/entity', format='csv')

# COMMAND ----------

dbutils.fs.ls(bimodels_path + 'entity/depth=1')
...
for r in tenants.select('name', 'id').collect():
  ...
  print(ent_tmp.count())
  ent_tmp.write.partitionBy('depth').save(bimodels_path + '/' + r.name + '/entity', format='csv', mode="overwrite")
  
  usage_tmp = usage_table.filter(usage_table.tenant_id == r.id)
  ent_tmp.write.save(bimodels_path + '/' + r.name + '/entity', format='csv', mode="overwrite")
