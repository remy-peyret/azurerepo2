...
logista_user = "81eaef0e-cf26-488f-beab-707f4c4a7a41-bluemix"
logista_pass = "b285e1dd5a571facf040532a629fa21538a78b1e84e9b005f0e8a710b14978a9"
logista_server = "https://81eaef0e-cf26-488f-beab-707f4c4a7a41-bluemix.cloudantnosqldb.appdomain.cloud"
...
logista_cloudant = get_cloudant_client(logista_user, logista_pass, logista_server)
...
#dbutils.fs.mount(
#  source = "adl://hxdatalakestorage.azuredatalakestore.net/postgresqlcopy_logista/",
#  mount_point = "/mnt/postgres-DLGen1-logista",
#  extra_configs = configs)

# COMMAND ----------

dbutils.fs.ls('/mnt/postgres-DLGen1-logista/')
...
dbutils.fs.mkdirs('/mnt/bi-models-DLGen1/Logista_space')

# COMMAND ----------

# DBTITLE 1,Create spark dataframes
all_tables = dbutils.fs.ls('/mnt/postgres-DLGen1-logista/')
for t in all_tables:
...
device = spark.read.format('csv').options(header='true', inferschema='true').load("/mnt/postgres-DLGen1-logista/public.core_device.txt")
entity = spark.read.format('csv').options(header='true', inferschema='true').load("/mnt/postgres-DLGen1-logista/public.core_entity.txt")
entitytype = spark.read.format('csv').options(header='true', inferschema='true').load("/mnt/postgres-DLGen1-logista/public.core_entitytype.txt")
equipment = spark.read.format('csv').options(header='true', inferschema='true').load("/mnt/postgres-DLGen1-logista/public.core_equipment.txt")
equipmenttemplate = spark.read.format('csv').options(header='true', inferschema='true').load("/mnt/postgres-DLGen1-logista/public.core_equipmenttemplate.txt")
fluidtype = spark.read.format('csv').options(header='true', inferschema='true').load("/mnt/postgres-DLGen1-logista/public.core_fluidtype.txt")
functionalgroup = spark.read.format('csv').options(header='true', inferschema='true').load("/mnt/postgres-DLGen1-logista/public.core_functionalgroup.txt")
location = spark.read.format('csv').options(header='true', inferschema='true').load("/mnt/postgres-DLGen1-logista/public.core_location.txt")
tag = spark.read.format('csv').options(header='true', inferschema='true').load("/mnt/postgres-DLGen1-logista/public.core_tag.txt")
transceiver = spark.read.format('csv').options(header='true', inferschema='true').load("/mnt/postgres-DLGen1-logista/public.core_transceiver.txt")
transceivertype = spark.read.format('csv').options(header='true', inferschema='true').load("/mnt/postgres-DLGen1-logista/public.core_transceivertype.txt")
unit = spark.read.format('csv').options(header='true', inferschema='true').load("/mnt/postgres-DLGen1-logista/public.core_unit.txt")
usage = spark.read.format('csv').options(header='true', inferschema='true').load("/mnt/postgres-DLGen1-logista/public.core_usage.txt")
usagetype = spark.read.format('csv').options(header='true', inferschema='true').load("/mnt/postgres-DLGen1-logista/public.core_usagetype.txt")
kpi = spark.read.format('csv').options(header='true', inferschema='true').load("/mnt/postgres-DLGen1-logista/public.kpi_kpi.txt")
...
usg_processed = logista_cloudant['usages-processed-data-id']
...
while (i - 1) * step <= doc_count:
  print(i)
  all_db_docs = usg_processed.all_docs(include_docs=True, skip = i * step, limit = step)
  ...
  all_data_df.union(sqlContext.read.json(myJson))
  i += 1
...
for r in tenants.select('name', 'id').collect():
  ...
  ent_tmp.write.option("header", "true").option("inferSchema", "true").option("delimiter", "\t").mode('overwrite').csv('/mnt/bi-models-DLGen1/Logista_space/' + r.name + '/entity')
  
  usage_tmp = usage_table.filter(usage_table.tenant_id == r.id)
  print("{0} usages".format(usage_tmp.count()))
  usage_tmp.write.option("header", "true").option("inferSchema", "true").option("delimiter", "\t").mode('overwrite').csv('/mnt/bi-models-DLGen1/Logista_space/' + r.name + '/usage')
  ...
  usage_temp_ext_tmp.write.option("header", "true").option("inferSchema", "true").option("delimiter", "\t").mode('overwrite').csv('/mnt/bi-models-DLGen1/Logista_space/' + r.name + '/usage_temp_ext')
  ...
  usage_conso_tmp.write.option("header", "true").option("inferSchema", "true").option("delimiter", "\t").mode('overwrite').csv('/mnt/bi-models-DLGen1/Logista_space/' + r.name + '/usage_conso')
  ...
  data_tmp.write.option("header", "true").option("inferSchema", "true").option("delimiter", "\t").mode('overwrite').csv('/mnt/bi-models-DLGen1/Logista_space/' + r.name + '/data')
  ...
  data_conso_tmp.write.option("header", "true").option("inferSchema", "true").option("delimiter", "\t").mode('overwrite').csv('/mnt/bi-models-DLGen1/Logista_space/' + r.name + '/data_conso')
  ...
  data_temp_ext_tmp.write.option("header", "true").option("inferSchema", "true").option("delimiter", "\t").mode('overwrite').csv('/mnt/bi-models-DLGen1/Logista_space/' + r.name + '/data_temp_ext')

  loc_tmp = location.filter(location.tenant_id == r.id)
  print("{0} locations".format(loc_tmp.count()))
  loc_tmp.write.option("header", "true").option("inferSchema", "true").option("delimiter", "\t").mode('overwrite').csv('/mnt/bi-models-DLGen1/Logista_space/' + r.name + '/loc')